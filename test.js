...
// === Gate A: Relay Detection Timing Tests ===

function testAnalyzeRelayTimingExport() {
  const { analyzeRelayTiming } = require('./index');
  assert.strictEqual(typeof analyzeRelayTiming, 'function');
  console.log('PASS: analyzeRelayTiming export');
}

function testGateAValidTiming() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1100 },
    { relay: 'R3', timestamp: 1200 },
    { relay: 'R4', timestamp: 1300 }
  ];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, true, 'evenly spaced events should be valid');
  assert.strictEqual(result.issues.length, 0, 'no issues expected');
  assert.strictEqual(result.summary.eventCount, 4);
  assert.strictEqual(result.summary.avgIntervalMs, 100);
  assert.strictEqual(result.summary.minIntervalMs, 100);
  assert.strictEqual(result.summary.maxIntervalMs, 100);
  console.log('PASS: Gate A valid timing');
}

function testGateANoEvents() {
  const { analyzeRelayTiming } = require('./index');
  const result = analyzeRelayTiming([]);
  assert.strictEqual(result.valid, false);
  assert.strictEqual(result.issues.length, 1);
  assert.strictEqual(result.issues[0].type, 'NO_EVENTS');
  assert.strictEqual(result.summary.eventCount, 0);
  console.log('PASS: Gate A no events');
}

function testGateANullEvents() {
  const { analyzeRelayTiming } = require('./index');
  const result = analyzeRelayTiming(null);
  assert.strictEqual(result.valid, false);
  assert.strictEqual(result.issues[0].type, 'NO_EVENTS');
  console.log('PASS: Gate A null events');
}

function testGateATooFast() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1010 },
    { relay: 'R3', timestamp: 1100 }
  ];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'too-fast interval should be invalid');
  const tooFast = result.issues.find(i => i.type === 'TOO_FAST');
  assert.ok(tooFast, 'must detect TOO_FAST issue');
  assert.strictEqual(tooFast.interval, 10);
  assert.strictEqual(tooFast.threshold, 50);
  console.log('PASS: Gate A too fast');
}

function testGateATooSlow() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1100 },
    { relay: 'R3', timestamp: 8000 }
  ];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'too-slow interval should be invalid');
  const tooSlow = result.issues.find(i => i.type === 'TOO_SLOW');
  assert.ok(tooSlow, 'must detect TOO_SLOW issue');
  assert.strictEqual(tooSlow.interval, 6900);
  assert.strictEqual(tooSlow.threshold, 5000);
  console.log('PASS: Gate A too slow');
}

function testGateAOutOfOrder() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R3', timestamp: 1200 },
  ...
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'out-of-order events should be invalid');
  const outOfOrder = result.issues.filter(i => i.type === 'OUT_OF_ORDER');
  assert.ok(outOfOrder.length > 0, 'must detect OUT_OF_ORDER issues');
  console.log('PASS: Gate A out of order');
}

function testGateADuplicateTimestamp() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1000 },
    { relay: 'R3', timestamp: 1100 }
  ];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'duplicate timestamps should be invalid');
  const dup = result.issues.find(i => i.type === 'DUPLICATE_TIMESTAMP');
  assert.ok(dup, 'must detect DUPLICATE_TIMESTAMP issue');
  assert.strictEqual(dup.timestamp, 1000);
  console.log('PASS: Gate A duplicate timestamp');
}

function testGateATimingDrift() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1100 },
    { relay: 'R3', timestamp: 1200 },
    { relay: 'R4', timestamp: 1800 }
  ];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'drifted timing should be invalid');
  const drift = result.issues.find(i => i.type === 'TIMING_DRIFT');
  assert.ok(drift, 'must detect TIMING_DRIFT issue');
  assert.ok(drift.drift > 200, 'drift must exceed threshold');
  console.log('PASS: Gate A timing drift');
}

function testGateAMalformedEvent() {
  const { analyzeRelayTiming } = require('./index');
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { bad: 'data' },
  ...
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, false, 'malformed events should be invalid');
  const invalid = result.issues.find(i => i.type === 'INVALID_EVENT');
  assert.ok(invalid, 'must detect INVALID_EVENT issue');
  assert.strictEqual(invalid.index, 1);
  console.log('PASS: Gate A malformed event');
}

function testGateACustomOptions() {
  const { analyzeRelayTiming } = require('./index');
  // With custom thresholds, 10ms interval is acceptable
  const events = [
    { relay: 'R1', timestamp: 1000 },
    { relay: 'R2', timestamp: 1010 }
  ];
  const result = analyzeRelayTiming(events, { minIntervalMs: 5, maxIntervalMs: 10000 });
  assert.strictEqual(result.valid, true, 'custom options should allow 10ms interval');
  assert.strictEqual(result.issues.length, 0);
  console.log('PASS: Gate A custom options');
}

function testGateASingleEvent() {
  const { analyzeRelayTiming } = require('./index');
  const events = [{ relay: 'R1', timestamp: 1000 }];
  const result = analyzeRelayTiming(events);
  assert.strictEqual(result.valid, true, 'single event should be valid (no intervals to check)');
  assert.strictEqual(result.issues.length, 0);
  assert.strictEqual(result.summary.eventCount, 1);
  console.log('PASS: Gate A single event');
}

function testGateAEndpointValid() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        const port = server.address().port;
        const { res, body } = await postJson(port, '/gate-a', {
          events: [
            { relay: 'R1', timestamp: 1000 },
            { relay: 'R2', timestamp: 1100 },
            { relay: 'R3', timestamp: 1200 }
        ...
        assert.strictEqual(res.statusCode, 200, 'valid timing should return 200');
        assert.strictEqual(body.valid, true);
        assert.strictEqual(body.issues.length, 0);
        assert.strictEqual(body.summary.eventCount, 3);
        console.log('PASS: Gate A endpoint valid');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
function testGateAEndpointInvalid() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        const { res, body } = await postJson(port, '/gate-a', {
          events: [
            { relay: 'R1', timestamp: 1000 },
            { relay: 'R2', timestamp: 1005 }
          ]
        });
        assert.strictEqual(res.statusCode, 422, 'timing issues should return 422');
        assert.strictEqual(body.valid, false);
        assert.ok(body.issues.length > 0, 'must report issues');
        console.log('PASS: Gate A endpoint invalid');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
function testGateAEndpointEmptyBody() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        const req = http.request({
          ...
          res.on('end', () => {
            try {
              ...
              assert.strictEqual(body.error, 'Request body is required');
              assert.strictEqual(body.code, 'BAD_REQUEST');
              console.log('PASS: Gate A endpoint empty body');
              resolve();
            } catch (err) {
              reject(err);
            } finally {
...
function testGateAEndpointWithCustomOptions() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        const { res, body } = await postJson(port, '/gate-a', {
          ...
          options: { minIntervalMs: 5 }
        });
        assert.strictEqual(res.statusCode, 200, 'custom min should allow 10ms');
        assert.strictEqual(body.valid, true);
        console.log('PASS: Gate A endpoint with custom options');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
(async () => {
  try {
    ...
    // Gate A: Relay Detection Timing
    testAnalyzeRelayTimingExport();
    testGateAValidTiming();
    testGateANoEvents();
    testGateANullEvents();
    testGateATooFast();
    testGateATooSlow();
    testGateAOutOfOrder();
    testGateADuplicateTimestamp();
    testGateATimingDrift();
    testGateAMalformedEvent();
    testGateACustomOptions();
    testGateASingleEvent();
    await testGateAEndpointValid();
    await testGateAEndpointInvalid();
    await testGateAEndpointEmptyBody();
    await testGateAEndpointWithCustomOptions();
