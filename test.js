...
function testBookmarkUrlTooLong() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        assert.strictEqual(body.status, 400);
        assert.strictEqual(body.code, 'VALIDATION_ERROR');
        assert.strictEqual(body.errors.url, 'URL exceeds maximum length of 2048 characters');
        console.log('PASS: bookmark URL too long rejected');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
function testBookmarkUrlExactly2048() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        assert.strictEqual(exactUrl.length, 2048, 'URL must be exactly 2048 chars');
        const { res, body } = await requestJson(port, 'POST', '/bookmarks', {
          url: exactUrl,
          title: 'Exact Limit URL'
        }, headers);
        assert.strictEqual(res.statusCode, 201);
        assert.strictEqual(body.url, exactUrl);
        assert.strictEqual(body.title, 'Exact Limit URL');
        console.log('PASS: bookmark URL exactly 2048 accepted');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
(async () => {
  try {
    ...
    await testBookmarkUrlTooLong();
    await testBookmarkUrlExactly2048();
