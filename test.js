...
(async () => {
  try {
    ...
    await testUptimeEndpoint();
    console.log('All tests passed');

function testUptimeEndpoint() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, () => {
      const port = server.address().port;
      http.get(`http://localhost:${port}/uptime`, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            assert.strictEqual(res.statusCode, 200);
            assert.ok(res.headers['content-type'].includes('application/json'), 'Content-Type must be application/json');
            assert.strictEqual(typeof body.uptime, 'number', 'uptime must be a number, not a string');
            assert.ok(body.uptime >= 0, 'uptime must be non-negative');
            assert.ok(Number.isFinite(body.uptime), 'uptime must be finite');
            console.log('PASS: uptime endpoint');
            resolve();
          } catch (err) {
            reject(err);
          } finally {
