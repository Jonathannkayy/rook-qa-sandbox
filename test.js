  ...
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        const { res } = await requestJson(port, 'DELETE', `/bookmarks/${created.id}`, undefined, headers);
        assert.strictEqual(res.statusCode, 204, 'DELETE must return 204 No Content');
        // Verify no response body
        const { res: listRes, body: listBody } = await requestJson(port, 'GET', '/bookmarks', undefined, headers);
        const found = listBody.find(b => b.id === created.id);
        assert.strictEqual(found, undefined, 'Deleted bookmark must not appear in list');
        console.log('PASS: DELETE /bookmarks/:id returns 204');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
  ...
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        assert.strictEqual(res.statusCode, 404, 'DELETE non-existent must return 404');
        assert.strictEqual(body.error, 'Bookmark not found');
        assert.strictEqual(body.code, 'NOT_FOUND');
        console.log('PASS: DELETE /bookmarks/:id non-existent returns 404');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
function testDeleteBookmarkRequiresAuth() {
  const app = require('./index');
  return new Promise((resolve, reject) => {
    const server = app.listen(0, async () => {
      try {
        ...
        assert.strictEqual(res.statusCode, 401, 'DELETE without auth must return 401');
        assert.strictEqual(body.code, 'AUTH_REQUIRED');
        console.log('PASS: DELETE /bookmarks/:id requires auth');
        resolve();
      } catch (err) {
        reject(err);
      } finally {
...
(async () => {
  try {
    ...
    await testDeleteBookmarkRequiresAuth();
